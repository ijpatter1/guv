...
# ── 1i. the hard-stop lingering-window warning reaches session-open context ────
# [32.2]: the [16.3] carrier that withdrew a stale CLAUDE_CODE_AUTO_COMPACT_WINDOW
# is deleted and guv never edits settings — surface WARNS instead, and this proves
# the warning actually rides the hook's additionalContext to the session (not just
# that surface emits it in isolation — context-management's W-suite covers that).
# Settings stay untouched and the frontier still surfaces alongside the warning.
RW=$(mkproj cw-hardstop-warn); tracker "$RW" open   # default fixture carries mode=hard-stop
...
CTXRW=$(echo "$OUT" | jq -r '.hookSpecificOutput.additionalContext // ""')
{ [ "$RC" -eq 0 ] && [ -z "$ERR" ]; } \
  && ok "session-start with a hard-stop window conflict: clean exit 0, stderr clean (a warning never blocks)" \
  || no "the context-wall warning must never block session start (rc=$RC err=$ERR)"
{ echo "$CTXRW" | grep -q 'CLAUDE_CODE_AUTO_COMPACT_WINDOW=250000' && echo "$CTXRW" | grep -q 'serial='; } \
  && ok "session-start surfaces the hard-stop lingering-window warning in context, frontier intact" \
  || no "the warning must reach additionalContext without suppressing the frontier (ctx=$CTXRW)"
{ [ "$WIN_AFTER" = "250000" ] && [ "$PERM_AFTER" = "Bash" ]; } \
  && ok "session-start warning path: settings untouched (guv discloses, never edits)" \
  || no "the warning path must not edit settings (win=$WIN_AFTER perm=$PERM_AFTER)"
