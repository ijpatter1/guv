...
#     already dual-reviewed in-band via /task + /eval, and the skip is
...
# The canonical procedure (the pointer target) must still exist in the /eval
...
if [ -f "$HANDOFF" ] && [ -f "$EVALUATE" ]; then
  ok "handoff.md and the eval skill both exist"
else
  no "handoff.md or eval SKILL.md missing"
  echo ""
...
# references that one definition rather than restating it. A bare `/eval`
# mention does NOT discriminate the deliverable — the pre-refactor handoff already
# named the `/eval` skill and `/eval-parallel` while still inlining the
# steps. The load-bearing invariant is the *don't-restate* pointer directive: the
# review procedure lives in /eval and handoff is told not to copy it. That
# instruction is absent from the inlined predecessor, so this fires on a regression.
echo "$HANDOFF_FLAT" | grep -qE '/eval' \
  && echo "$HANDOFF_FLAT" | grep -qiE 'do not restate' \
  && ok "handoff points at /eval as the one definition and forbids restating its procedure" \
  || no "handoff must point at /eval and direct that its procedure not be restated (no inline copy)"

# T3 — no inlined copy of /eval's review steps. The drift surface was the
# two prose passages that restated, verbatim-ish, /eval's Step 2/3 invoke
# instructions ("invoke the `evaluator` subagent using the Agent tool" and
# "Invoke the `reviewer` subagent using the Agent tool"). The deliverable
# kills exactly that restatement, so neither inline invoke-instruction may remain
# in handoff.md — they belong to /eval now.
...
if echo "$HANDOFF_FLAT" | grep -qiE 'Invoke the .?reviewer.? subagent using the Agent tool'; then
  no "handoff still inlines the 'invoke the reviewer subagent' step (drift surface)"
else
  ok "handoff no longer inlines the reviewer-invocation step"
fi

# T4 — the skip condition is documented: the session-close review is SKIPPED when
# every session commit was already dual-reviewed in-band via /task + /eval.
...
echo "$HANDOFF_FLAT" | grep -qiE 'in-band' \
  ...
  || no "the skip precondition must reference in-band review (via /task + /eval)"
