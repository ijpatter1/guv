...
if [ -n "$TRANSCRIPT" ]; then
  ...
  # DEDUPE BY requestId — one API response counts ONCE (lockstep with meter.sh;
  # the rationale and the measured 2.55x inflation are documented there). A
  # response is serialized as N lines, one per content block, carrying duplicate
  # usage in one of two forms (66.2% identical repeats, 33.8% placeholder-until-
  # final); max per class is correct under both serializations,
  # and a key-less line gets a per-line synthetic key so nothing is collapsed.
