...
- **`spike`** — exploration mode, not a scoped-change door. A spike project also has
  ...
  `$ARGUMENTS`, `/task` stays content-driven (see the routing note below) and
  processes it. But with no specific change — a session-entry landing in a spike
  project — name `/spike` (free-form exploration, no phase DAG) and stop, rather than
...
`/spike` (free-form exploration), not a scoped change — name `/spike` and stop, so
...
to `/spike` (the spike branch above), not a request for a scoped change.
...
6. Run the review gate (`/eval`) on the change — grade what it surfaces: fix what changes a decision, record the rest
...
3. Run the review gate (`/eval`) — grade findings, fix what changes a decision, record the rest. Note in the session handoff under **Completed** with a reference to the doc changes
...
3. Run the review gate (`/eval`) — grade findings, fix what changes a decision, record the rest. Commit with `feat(scope): description`
