...
`docs/spikes/` — the `/spike` skill's close (Step 5) points here for exactly that.
...
The build set, named concretely for `/replan insert` — each item with a *candidate*
...
gated build set is groomed via `/replan` and its gating deliverable flips ✅. A
