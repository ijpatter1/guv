...
- **Exit 4 (pre-scaffold)** — no manifest here yet: there is nothing to explore in
  ...
  `/onboard` for an existing repo, `/init` for a spec — and **stop**.
...
- **Drain it** to the destination you named in Step 3. The finding's written **home**
  ...
  - **Gates build work → `/plan` or `/replan insert`, *plus* the `docs/spikes/`
    ...
    did both — a `docs/spikes/` note *and* a `/replan` groom).
  - **Friction with guv → `/feedback`** (a separate channel — `feedback.ndjson`, not
