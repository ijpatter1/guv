...
- Phase placement carries no ordering ([7.6]): `[deps: none]` means
  *dispatchable immediately*, full stop — across phases. If a deliverable
  ...
  duty — plan generators and `/replan` inserts are held to this bar)
...
- Every mutation of a live tracker goes through `/replan`, the single
...
- A phase with all deliverables ✅/❌ auto-tallies complete — *except* a
  ...
  **phase-close** step (`bash .claude/replan.sh phase-close <session> <phase>`),
...
`OP` is the `/replan` verb (reorder, split, merge, insert, descope, abandon,
...
`.claude/resolve-ready.sh` computes the ready frontier from a tracker
(`bash .claude/resolve-ready.sh [tracker-path]`, default
...
- **Archive layout:** a completed (or explicitly abandoned) initiative's
  ...
  `.claude/archive-initiative.sh` — use it rather than hand-moving files.
