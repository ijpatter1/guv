...
deliberately with an explicit **phase-close** step — `bash .claude/replan.sh
...
- **For an insert:** query the ordinal first — `bash .claude/replan.sh next-ordinal <phase>` —
  ...
  0.5 / heavy ≈ 0.9** (`bash .claude/estimate.sh rubric`). If the scope would exceed
  **one** session it is a **balloon** — **SPLIT it** into deliverables that each fit one
  session (a split *is* a `/replan`: reword the original to its narrowed scope + insert
...
`/plan` does at plan time. **This is a hard gate: no document is written
...
4. **The estimate sidecar** ([9.6], [13.2], inserts only) — record the sizing the user
   ...
   **leaves the tracker byte-identical**, which is exactly why the estimate does *not*
   ...
   estimate revision is a bare `estimate.sh set-sized` (or legacy `set`), no `/replan` needed.
...
- Spec-alignment gaps found at session start route here too (`/phase` Step 5
