...
spec-alignment pass). When you're *crossing into a new phase*, use `/phase`
instead — that door does the full sequence. Which door applies is decided
deterministically by the router (`.claude/route.sh`, [8.1]): Step 0 defers to
...
- If `ceremony` is **`spike`**, this is exploration mode — there is no plan DAG to
  resume. That is a **mode signal, not an error**: name `/spike` (free-form
  ...
  without it the catch-all would misname `/task`.
...
Deliberately **not** done here (it's the `/phase` boundary ritual): the
