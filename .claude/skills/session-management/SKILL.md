...
- **Include commit hashes.** The next session can use these for `git show` / `git diff` against the **code** repo (`bash .claude/guv-git.sh show <hash>`, a no-op for single-repo) to quickly inspect specific changes. Note that doc/session commits live in the control plane and product commits in the code repo, so cite hashes with their repo when the roots differ.
...
- When a phase is fully complete (all deliverables ✅), add a completion date to the phase header. **Lone-deliverable carve ([15.7]):** a phase holding a *single* deliverable is "fully complete" only once it is **SEALED** — sealed by an explicit `phase-close` record (`bash .claude/replan.sh phase-close <session> <phase>`), not merely by its one deliverable going ✅. This mirrors the engine (`replan.sh` `phase_completed`/`phase_sealed`) and its sibling oracles (`archive-initiative.sh`, the `handoff` skill's phase-completion step): a lone gating spike flipped ✅ before its gated build set is groomed in is **open until sealed**, so do not stamp a completion date on it until the seal record exists. The **multi-deliverable** rule is unchanged — ≥2 deliverables, all ✅, is fully complete with no manual seal
...
1. Find the current phase (the one with deliverables that aren't all ✅) — and note the lone-deliverable carve ([15.7]): a phase whose *single* deliverable is ✅ but that is **not** yet sealed (no `phase-close` record) is still the **current/open** phase, not a completed one, so it stays mutable until sealed (mirrors the engine and its sibling oracles; the multi-deliverable rule is unchanged)
...
Just run `bash .claude/guv-git.sh log --oneline -10` and the project's test command (`bash .claude/guv-cmd.sh test`) to reorient. The session artifact from earlier today has the full context.
...
2. Ensure the completed phase branch has been merged to `main` and the merge is clean. Phase branches live in the **code** repo, so run branch ops there (a no-op for single-repo): `bash .claude/guv-git.sh checkout main && bash .claude/guv-git.sh merge phase/N-previous-name`
3. Read the new phase's section in `docs/REQUIREMENTS.md` thoroughly
4. Read relevant sections of `docs/ARCHITECTURE.md`
5. Create the new phase branch from `main` in the code repo: `bash .claude/guv-git.sh checkout -b phase/N-description`
