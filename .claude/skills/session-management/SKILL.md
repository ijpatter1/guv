...
Use `/start-phase N` and also read the 2-3 most recent session artifacts, not just the latest. Check `bash .claude/guv-git.sh log --oneline -30` for a broader view of recent progress. Run the project's test and build commands (`bash .claude/guv-cmd.sh test` / `bash .claude/guv-cmd.sh build` — each skips loudly when null) to confirm the codebase is healthy.
