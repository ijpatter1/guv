...
HELPERS="archive-initiative resolve-ready replan resolve-stack check-citations update-readme-status"
...
rewrite_paths() {
  sed -E 's|\.claude/(hooks/)?(archive-initiative\|resolve-ready\|replan\|resolve-stack\|check-citations\|update-readme-status\|bash-guard\|auto-format\|stop-check)\.sh|"${CLAUDE_PLUGIN_ROOT}"/scripts/\2.sh|g'
}
