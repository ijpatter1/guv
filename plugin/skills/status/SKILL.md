...
2. Run `bash "${CLAUDE_PLUGIN_ROOT}"/scripts/guv-git.sh log --oneline -5` for recent code activity (if git is initialized; otherwise note "no git history"). The helper targets the code repo (`roots.code`, `"."` for single-repo, so this is a no-op there).
...
4. Check `bash "${CLAUDE_PLUGIN_ROOT}"/scripts/guv-git.sh status` for any uncommitted code changes
...
7. Count open guv-feedback entries: `f=.claude/feedback/feedback.ndjson; [ -f "$f" ] && jq -s '[.[] | select(.status=="open")] | length' "$f" || echo 0`.
8. **Context-wall posture ([16.2]).** Run `bash "${CLAUDE_PLUGIN_ROOT}"/scripts/context-management.sh surface .claude/project.json` — the same read-mostly surfacer the session-start hook uses. It emits the loud `context-wall mode UNSET` marker for a fresh headless scaffold that never chose a mode, the one-time migration nudge for a block-less in-field project, or **nothing** for a configured posture (or an absent helper/manifest). Capture its output; it never blocks (Rule 15). This is the status half of the marker's surfacing — the door prose promises it surfaces "at session-open _and_ in the status report," so a `/guv:status` run is the second place an unconfigured posture must show up.
9. **README status block — no hand-invoke needed.** In `phased` projects the block is refreshed automatically by the §3.3 render hooks — the native PostToolUse hook when a tracker edit lands, and the control plane's git post-commit hook on every tracker commit — both deriving the line from the resolver (`status-line.sh`, never a second source of truth). `/guv:status` only reads state here; there is nothing to write. (In `task`/`onboard` mode there is no tracker to derive from, and a consumer README normally carries no STATUS markers, so there is nothing to refresh.)
...
- **Open guv feedback:** _include this line only if the count from step 7 is > 0_ — "N open (triage with the `feedback` skill)". If 0, omit entirely.
- **Context-wall:** _include this line only if step 8 emitted anything_ — surface the `context-wall mode UNSET` marker (or the migration nudge) it printed, verbatim. If it was silent (a configured posture, or pre-feature project already nudged), omit this line entirely.
