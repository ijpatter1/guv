...
| `artifact`   |     | The implicated file+line or command, e.g. `.claude/rules/guv-verification.md:7` or `/guv:phase` (omit if not file-specific)                                                                                                                                                                                                                                      |
...
command). The inline jq below is kept only for the read-only queries `/guv:status`
and `/guv:handoff` call programmatically.
...
# graduate — the provenance note is REQUIRED (this is the form /guv:handoff's drain step uses):
