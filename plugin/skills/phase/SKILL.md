...
have the context, use the light `/guv:next` door instead — it reads the resolver's
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}"/scripts/route.sh --for phase
```
...
- **Exit 4 (pre-scaffold)** — no manifest here yet: there is no phase to enter.
  ...
  tell the user to scaffold first — `/guv:init` for a spec, `/guv:onboard` for an
...
**If a spec exists:** Invoke the `guv:reviewer` subagent with a targeted prompt:
