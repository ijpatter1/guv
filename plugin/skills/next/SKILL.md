...
- **Exit 4 (pre-scaffold)** — no manifest here yet: there is no plan to resume.
  ...
  user to scaffold first — `/guv:onboard` for an existing repo, `/guv:init` for a
...
- **Fan-out offer (non-blocking):** when the frontier holds two or more ready
  ...
  `bash "${CLAUDE_PLUGIN_ROOT}"/scripts/resolve-ready.sh | bash "${CLAUDE_PLUGIN_ROOT}"/scripts/fanout-offer.sh -` — which reports
  ...
  call explicitly — **fan out** (hand the disjoint, sized set to `/guv:build-fanout
  ...
  `bash "${CLAUDE_PLUGIN_ROOT}"/scripts/estimate.sh set-sized <id> <light|medium|heavy>` — an estimate
  revision needs no `/guv:replan` — and **split** a `balloons=` candidate via `/guv:replan`
