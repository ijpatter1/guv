...
A lane is a git worktree of a code repo, and a lane builder runs `/guv:task` *inside* it —
...
```
bash "${CLAUDE_PLUGIN_ROOT}"/scripts/provision-code-repo.sh <code-repo-path> --test "<the repo's test command>"
```
...
split (`/guv:replan`) before the fan-out, not pushed into a lane that will overflow.
...
2. Spawn the calibrated **`@guv:lane-builder`** agent (BY NAME, Rule 14 — not an ad-hoc
   ...
   - acquires the behavioral core **natively** — an Agent-tool subagent inherits this
     ...
     mechanism: native inheritance + per-lane `/guv:task`, not hand-carried conventions);
   - builds via `/guv:task` — **red→green TDD**, confined to source, commit sources only;
...
```
bash "${CLAUDE_PLUGIN_ROOT}"/scripts/lane-recovery.sh assess <id> --dir <worktree>
```
...
the held lane in its worktree, then re-run `/guv:build-fanout` for the next pass. Iterate to
...
- **the trackers** (`docs/PHASE_STATUS.md`, `docs/REQUIREMENTS.md`) — plan mutation is
  `/guv:replan` only, never a lane edit;
...
`provision-code-repo.sh` (once per repo) → `guv-lane.sh create` + `@guv:lane-builder` via
`/guv:task` (execute) → `/guv:build-fanout` (gate, by name) → fix conversationally + re-gate to
