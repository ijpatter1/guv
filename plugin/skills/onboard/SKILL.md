...
Use this for code that **already exists**. Unlike `/guv:init` (greenfield, full
...
> **`/guv:onboard` supersedes Claude Code's built-in `/init` in guv projects.** The
...
> built-in is distinct from guv's `/guv:init` (the greenfield door, whose bare
...
- **The repo's README is the guv template README** (e.g. it still says "Claude Code
  ...
  render `README.template.md` → `README.md` as `/guv:init` does, describing what's
