---
name: replan
description: "Mutate the live plan through the one sanctioned door: classify the requested change, confirm it with the user, then apply it atomically across the three phase docs with an amendment record."
user-invocable: true
---
...
The change being requested. If empty, ask the user what plan change they want
before proceeding.
...
DAG to mutate — name `/guv:spike` (free-form exploration) and stop. Likewise in `task`
...
- **`mode=LEGACY`:** token-free trackers predate the DAG grammar and `/guv:replan`
...
- **For an insert:** query the ordinal first — `bash "${CLAUDE_PLUGIN_ROOT}"/scripts/replan.sh next-ordinal <phase>` —
...
- **For a new phase (the section header):** the engine inserts *deliverables* but
  ...
  tracker with `bash "${CLAUDE_PLUGIN_ROOT}"/scripts/replan.sh sync-check` and the resolver — a malformed
...
- Check the target is mutable before going further: `bash "${CLAUDE_PLUGIN_ROOT}"/scripts/replan.sh guard <ID-or-phase>`.
...
2. **`docs/PHASE_STATUS.md`** — apply the same change through the engine (never by
   ...
   ```
   bash "${CLAUDE_PLUGIN_ROOT}"/scripts/replan.sh insert  <session-id> <verb> '<full wording>'
   bash "${CLAUDE_PLUGIN_ROOT}"/scripts/replan.sh descope <session-id> <verb> <ID> '<note>'
   bash "${CLAUDE_PLUGIN_ROOT}"/scripts/replan.sh reword  <session-id> <verb> <ID> '<full wording>' '' '<what changed>'
   ```
...
1. Verify the verbatim-sync contract on every touched ID:
   ...
   ```
   bash "${CLAUDE_PLUGIN_ROOT}"/scripts/replan.sh sync-check <ID>
   ```

   Non-zero means tracker and REQUIREMENTS drifted mid-apply — fix before reporting.
2. Re-run `bash "${CLAUDE_PLUGIN_ROOT}"/scripts/resolve-ready.sh` and present the new frontier — a plan
...
- One operation per confirmation. A batch of unrelated changes is several `/guv:replan`
