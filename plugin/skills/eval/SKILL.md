...
technical findings, and the `guv:reviewer` subagent for alignment with the spec and
vision. Findings, not scores. ([32.1]; S2/S3, spec-2026-07-31.)

## When to Use

- At a change's commit gate (the `/guv:task` paths invoke this)
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}"/scripts/guv-git.sh log --oneline -10   # recent commits (roots.code)
bash "${CLAUDE_PLUGIN_ROOT}"/scripts/guv-git.sh diff HEAD~N          # the diff under review
...
Invoke the `guv:reviewer` subagent **by name**, worktree-isolated, with a prompt like:
