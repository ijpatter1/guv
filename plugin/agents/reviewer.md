...
- You do NOT run the test battery. Test state is the evaluator's result, not yours — and on a project with a long suite the battery is the most expensive thing in a QA pass, so exactly one stage pays for it. If you need to know whether the tree is green, read the recorded verdict (`bash "${CLAUDE_PLUGIN_ROOT}"/scripts/battery-result.sh read`) and quote it in the unit it labels (`suites:` are suites, `assertions:` are tests); a non-zero exit means no fresh whole-tree proof exists, which you report as such rather than producing one.
