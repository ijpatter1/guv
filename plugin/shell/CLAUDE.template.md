...
- **Process commands:** `/task` (scoped change), `/onboard` (adopt an existing repo), `/init-project` (greenfield setup), `/plan-initiative` (multi-phase initiative on an existing project), then `/start-phase`, `/evaluate`, `/handoff`, `/status`, `/manual`. The commands carry the repeatable procedure; follow their steps. (Installed as the guv plugin, every name carries the namespace: `/guv:task`, `/guv:handoff`, ….)
