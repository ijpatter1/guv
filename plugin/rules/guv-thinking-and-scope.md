...
- **The commands** (`/start-phase`, `/task`, `/handoff`, … — `/guv:`-namespaced under a plugin install) govern *process* — repeatable workflows. When a command gives numbered steps, follow them; Rule 4 is about tasks the commands *don't* script, not a license to skip a command's procedure.
