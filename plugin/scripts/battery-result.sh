...
case "$CMD" in
  record)
    [ $# -ge 4 ] || die "record needs <rc> <suites> <passed> <failed> [<only-pattern>]" 2
    rc="$1"; suites="$2"; passed="$3"; failed="$4"; only="${5:-}"
    fp=$(fingerprint)
    ...
    echo "battery-result: recorded ($passed passed, $failed failed) against $(git -C "$CODE" rev-parse --short HEAD)${only:+ [filtered: $only]}"
    ;;
