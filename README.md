...
**Repo topology** — two shapes, chosen per project. **Single-repo** (`roots` both `"."`) is the current scaffold default and suits internal apps, where plan and product share one tree and every root-aware operation collapses to a no-op. **Split** — a control plane plus a sibling code repo — is _recommended for standalone or publishable products_: it keeps the plan, sessions, and governance (`.claude/`) cleanly separate from the shippable code, and it's the foundation the control-plane model builds on (multi-repo / N-repo support is the planned next step). In a split, Claude launches in the control plane and the product is a sibling repo. Convention: the code repo keeps the plain product name, the control plane is its sibling named per the `<project>-guv` convention (here `<product>-guv` — a possessive suffix, the product's guv; human-facing only, never used for discovery: the manifest is the sole machine pointer), and the manifest's `name` stays the _product_ name (it feeds image/container labels):
