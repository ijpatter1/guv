...
- **Execution at scale:** saved dynamic workflows — project `.claude/workflows/` (e.g. `/build-fanout`) on a template install, or the plugin's `/guv:build-fanout`. The planning layer is the phase docs and the commands; the execution layer is the model, subagents, and — for wide mechanical fan-out — workflows. Workflow QA gates use the platform review plus the `reviewer` by name, and ultracode is fan-out-only, dropped back after (`.claude/rules/guv-workflows.md`).
