...
- **Execution at scale:** saved dynamic workflows in `.claude/workflows/` (e.g. `/evaluate-parallel`). The planning layer is the phase docs and the commands; the execution layer is the model, subagents, and — for wide mechanical fan-out — workflows. Workflow QA stages invoke the calibrated reviewers by name, and ultracode is fan-out-only, dropped back after (`.claude/rules/guv-workflows.md`).
