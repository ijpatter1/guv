...
- **Process commands:** `/task` (scoped change), `/onboard` (adopt an existing repo), `/init-project` (greenfield setup), `/plan` (multi-phase initiative on an existing project), then `/phase` (phase-boundary entry), `/next` (light daily/mid-phase resume), `/replan` (plan mutation — the one sanctioned door), `/eval`, `/handoff`, `/status`, `/manual`. The commands carry the repeatable procedure; follow their steps. (Installed as the guv plugin, every name carries the namespace: `/guv:task`, `/guv:handoff`, ….)
...
- **Execution at scale:** saved dynamic workflows — project `.claude/workflows/` (e.g. `/eval-parallel`) on a template install, or the plugin's `/guv:eval-parallel`. The planning layer is the phase docs and the commands; the execution layer is the model, subagents, and — for wide mechanical fan-out — workflows. Workflow QA stages invoke the calibrated reviewers by name, and ultracode is fan-out-only, dropped back after (`.claude/rules/guv-workflows.md`).
